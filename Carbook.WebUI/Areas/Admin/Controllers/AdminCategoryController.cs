...
namespace Carbook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminCategoryController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7076/api/Categories", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7076/api/Categories/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminCategory", new { area = "Admin" });
        ...
        public async Task<IActionResult> UpdateCategory(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7076/api/Categories/{id}");
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryDto updateCategoryDto)
        {
            ...
            var content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7076/api/Categories", content);
            if (responseMessage.IsSuccessStatusCode)
            {
