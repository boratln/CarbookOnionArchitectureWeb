...
namespace Carbook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminLocationController : Controller
    {
        ...
        public async Task<IActionResult> CreateLocation(CreateLocationDto createLocationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7076/api/Locations", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7076/api/Locations/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminLocation", new { area = "Admin" });
        ...
        public async Task<IActionResult> UpdateLocation(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7076/api/Locations/{id}");
            ...
            return View(new UpdateLocationDto());
        }
        [HttpPost]
        [Route("UpdateLocation/{id}")]
        public async Task<IActionResult> UpdateLocation(UpdateLocationDto updateLocationDto)
        {
            int id = (int)HttpContext.Session.GetInt32("Locationid");

            updateLocationDto.LocationId = id;
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(updateLocationDto);
            var content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7076/api/Locations", content);
            if (responseMessage.IsSuccessStatusCode)
            {
